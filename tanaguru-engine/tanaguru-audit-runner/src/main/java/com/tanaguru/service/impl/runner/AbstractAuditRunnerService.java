...
public abstract class AbstractAuditRunnerService implements AuditRunnerListener, AuditRunnerService {
	...
	public AbstractAuditRunnerService(PageRepository pageRepository, AuditRepository auditRepository,
			AuditService auditService, PageContentRepository pageContentRepository,
			TestResultRepository testResultRepository, TestHierarchyResultRepository testHierarchyResultRepository,
			ResultAnalyzerService resultAnalyzerService, TestHierarchyRepository testHierarchyRepository,
			ElementResultRepository elementResultRepository, MailService mailService, MessageService messageService,
			ActRepository actRepository, ContractUserRepository contractUserRepository,
	...
	@Override
	public final void onAuditNewPage(AuditRunner auditRunner, String name, String url, int rank,
			WebextPageResult result, String screenshot, String source) {
	...
	public final void onAuditEnd(AuditRunner auditRunner) {
		...
		if(pages.isEmpty()) {
			...
			auditService.log(auditRunner.getAudit(), EAuditLogLevel.ERROR,
					"Audit failed because it does not contain pages");
		} else {
		...
		if(audit.getType().equals(EAuditType.SITE) || audit.getType().equals(EAuditType.SCENARIO)
				|| pages.size() >= 2) {
			Audit finalAudit = audit;
			actRepository.findByAudit(audit).ifPresent(act1 -> {
				Collection<ProjectAppUser> projectAppUsers = projectUserRepository.findAllByProjectAndMailEnabledIsTrue(act1.getProject());
				...
				for(ProjectAppUser projectAppUser: projectAppUsers) {
					...
					try {
						boolean emailSent = mailService.sendMimeMessage(user.getEmail(),
								messageService.getMessage("mail.auditEnd.subject"),
								messageService.getMessage("mail.auditEnd.body").replace("domain", domain)
										.replaceAll("url", url));

						if(emailSent) {
							auditService.log(auditRunner.getAudit(), EAuditLogLevel.INFO,
									"E-mail notifying the end of the audit sent to " + user.getEmail());
						} else {
							auditService.log(auditRunner.getAudit(), EAuditLogLevel.ERROR,
									"Failed to send email at the end of the audit to " + user.getEmail());
						}

					}
					catch(MailException e) {
						LOGGER.error("[Audit {}] Failed to send email at the end of the audit to {}. Erreur: {}",
								finalAudit.getId(), user.getEmail(), e);
						auditService.log(auditRunner.getAudit(), EAuditLogLevel.ERROR,
								"Failed to send email at the end of the audit to " + user.getEmail());
					}
