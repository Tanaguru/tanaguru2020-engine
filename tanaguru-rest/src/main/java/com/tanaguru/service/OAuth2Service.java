...
public class OAuth2Service {
	...
	private boolean expirationTimeIdTokenIsValid(IdTokenDTO idToken) {
		...
		Date expirationTime = new java.util.Date(idToken.getPayload().getExp() * 1000);
	...
	private boolean algAndKidIdTokenIsValid(IdTokenDTO idToken) {
		...
		try {
			...
			JSONObject lastKey = publicKeys.getJSONArray("keys").getJSONObject(0);
			String kid = lastKey.getString("kid");
			final Boolean hasAlg = lastKey.has("alg");

			if(kid.equals(idToken.getJoseHeader().getKid())
					&& (!hasAlg || lastKey.getString("alg").equals(idToken.getJoseHeader().getAlg()))) {
				algAndKidValid = true;
			}
	...
	private void createSimpleUserWithEmail(String email) {
		...
		userService.createUser(email, email, passwordEncoder.encode(pswd), EAppRole.USER, true, userCreateContract,
				null, null, EAppAccountType.DEFAULT);
	}
